...
func createFolder(c *gin.Context) {
	...
	// 再将文件夹与User关联
	userFile, err := models.InsertUserFile(userID.(uint), folder.ID, form.FolderName, form.Path, true)
	if err != nil {
		c.JSON(http.StatusOK, gin.H{"meta": models.Meta{1, "创建文件夹失败！！！"}})
		return
	}
	c.JSON(http.StatusOK, gin.H{"meta": models.Meta{0, "success"}, "file": userFile})
}
